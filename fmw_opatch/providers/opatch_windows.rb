...
action :apply do
  ...
  if @current_resource.exists
    ...
    converge_by("Create resource #{ @new_resource }") do
      result = false
      shell_out!("#{@new_resource.oracle_home_dir}\\OPatch\\opatch.bat apply -silent -jdk #{@new_resource.java_home_dir} -oh #{@new_resource.oracle_home_dir} #{@new_resource.tmp_dir}/#{@new_resource.patch_id} -oop", :timeout => 1200).stdout.each_line do |line|
        unless line.nil?
          Chef::Log.info(line)
